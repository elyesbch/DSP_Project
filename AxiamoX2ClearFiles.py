...
class FTPClearer:
    ...
    def run(self):
        ...
        print(f"FTP remove files complete? {success}")
        self.running=False
...
if __name__ == "__main__":
    ...
    clearer = FTPClearer(address, "axiamo", "axiamo", "rawdata", name, False)

    clearer.run()

    while(clearer.running):
        print(f"Axiamo X2 Clear Files running {time.time()}")
        time.sleep(1)