from axiamo_lib.AxiamoX2BLE import *
...
class Test_Composer:
    def test_composeWifiCred(self):
        composer = X2composer()
        cmd = composer.composeWifiCred("abc", "123")
        expected_abc123 = bytearray(
            [0x7D, 0x08, 0x00, 0x61, 0x62, 0x63, 0x09, 0x31, 0x32, 0x33, 0xBB, 0x4F]
        )
        Logger.info(f"Expected: {binascii.hexlify(expected_abc123)}")
        Logger.info(f"Actual:  {binascii.hexlify(cmd)}")
        assert cmd == expected_abc123

    def test_composeConsumersMask(self):
        composer = X2composer()

        producers = 1 << composer.IndexIMU  # which producers we want to consume
        cmd = composer.composeConsumersMask(producers)
        expected = bytearray([0x7D, 0x04, 0x07, 0x02, 0x08, 0x00, 0x99, 0x5C])
        Logger.info(f"Expected: {binascii.hexlify(expected)}")
        Logger.info(f"Actual:  {binascii.hexlify(cmd)}")
        assert cmd == expected

        producers = 1 << composer.IndexIMU | (
            1 << composer.IndexPing
        )  # which producers we want to consume
        cmd = composer.composeConsumersMask(producers)
        expected = bytearray([0x7D, 0x04, 0x07, 0x02, 0x28, 0x00, 0x59, 0x45])
        Logger.info(f"Expected: {binascii.hexlify(expected)}")
        Logger.info(f"Actual:  {binascii.hexlify(cmd)}")
        assert cmd == expected
